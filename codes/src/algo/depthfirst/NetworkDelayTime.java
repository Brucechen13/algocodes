...
public class NetworkDelayTime {
    class MapNode implements Comparable<MapNode>{
        ...
        int end;
        int weight;
        public MapNode(int start, int end, int weight){
    ...
    public void dfs(int[][] mat, int index, int[] seqTimes, int preTime){

        for(int i = 0; i < mat[index].length; i ++){
            if(mat[index][i] == 0)continue;
            if(seqTimes[i] == 0 || seqTimes[i] > mat[index][i]){
                seqTimes[i] = mat[index][i];
                dfs(mat, i, seqTimes, preTime + mat[index][i]);
            }
        }
    }
}
